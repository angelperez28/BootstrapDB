  ...
  def HasToken[A](p: BodyParser[A] = parse.anyContent)(f: String => Long => Request[A] => Result): Action[A] =
    Action(p) { implicit request =>
      request.cookies.get("XSRF-TOKEN").fold {
        ...
        Redirect(routes.Application.login()).flashing("message" -> "Invalid XSRF Token cookie")
      } { xsrfTokenCookie =>
        ...
        maybeToken flatMap { token =>
          Cache.getAs[Long](token) map { userId =>
            if (xsrfTokenCookie.value.equals(token)) {
              ...
              Redirect(routes.Application.login()).flashing("message" -> "Invalid Token")
            }
          }
        } getOrElse {
          //Unauthorized(Json.obj("message" -> "No Token"))
          Redirect(routes.Application.login()).flashing("message" -> "No Token")
        }
      }
    }
